...
- 🔌 **WebSocket 支持** - 完整的 WebSocket 代理支持，包括连接日志和消息记录
- 📡 **SSE 支持** - Server-Sent Events 代理支持，实时事件流处理
...
| `wsMiddleware`      | `WebSocketMiddleware[]`   | `[]`             | WebSocket 中间件数组 |
| `sseMiddleware`     | `SSEMiddleware[]`        | `[]`             | SSE 中间件数组     |
| `webSocketFilter`   | `WebSocketFilter`        | -                | WebSocket 过滤器   |
| `customProxyConfig` | `Partial<ProxyOptions>`  | -                | 自定义代理配置     |
| `rewriteRules`      | `Record<string, string>` | -                | URL重写规则        |
| `webSocket`         | `WebSocketConfig`        | -                | WebSocket 全局配置 |
...
| `showWsConnections`   | `boolean`  | `false`         | 显示 WebSocket 连接日志 |
| `showWsMessages`      | `boolean`  | `false`         | 显示 WebSocket 消息日志 |
| `maxWsMessageLength`  | `number`   | `1000`          | WebSocket 消息最大长度 |
| `showSseConnections`  | `boolean`  | `false`         | 显示 SSE 连接日志   |
| `showSseMessages`     | `boolean`  | `false`         | 显示 SSE 消息日志   |
| `maxSseMessageLength` | `number`   | `1000`          | SSE 消息最大长度    |
...
- 注意：`defineProxyConfig` 函数主要用于类型提示，不是必需的。你可以直接 `export default { ... }`。
...
- `logWebSocketConnection(url, protocols?)` - 记录 WebSocket 连接
- `logWebSocketMessage(url, message, direction)` - 记录 WebSocket 消息
- `logSSEConnection(method, url)` - 记录 SSE 连接
- `logSSEMessage(url, message)` - 记录 SSE 消息
...
- `prettifyMessages` - 是否美化 JSON 消息（默认：`true`）
- `headers` - 自定义 WebSocket 头部
...
- `maxMessageLength` - 最大消息日志长度（默认：`1000`）
- `prettifyMessages` - 是否美化 JSON 消息（默认：`true`）
- `headers` - 自定义 SSE 响应头部
- `retryInterval` - SSE 重连间隔（毫秒，默认：`3000`）
